...
function DaNhan({ navigation }) {
  ...
  async function getReceivedFriendRequests() {
    try {
      const res = await axiosPrivate.get(`/friendRequest/received/${currentUser.user.uid}`);
  ...
  async function TuChoi(id) {
    try {
      await axiosPrivate.post(`/friendRequest/decline`, { friendRequestId: id });
      getReceivedFriendRequests();
    } catch (error) {
      console.log(error);
    }
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={{backgroundColor:'#fff', flexDirection: "row",padding:7,width:'100%',height:97, borderBottomWidth:0.5}}>
            <Image
            ...
            <View style={{ marginLeft: 10,gap:4 }}>
              ...
              <Text style={{ fontSize:15,color:'#767A7F' }}>{(covertTime(item.createdAt))}</Text>
              <View style={{flexDirection:'row',gap:8}}>
                <TouchableOpacity onPress={()=>{
                  TuChoi(item._id)
                }} style={{backgroundColor:'#E0E0E0',width:110,height:32,borderRadius:15,justifyContent:'center',alignItems:'center'}}>
                  <Text style={{color:'black',fontSize:16}}>Từ chối</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={()=>{
                  ChapNhan(item._id)
                }} style={{backgroundColor:'#F0F9FC',width:110,height:32,borderRadius:15,justifyContent:'center',alignItems:'center'}}>
                  <Text style={{color:'#20B2AA',fontSize:16}}>Chấp nhận</Text>
                </TouchableOpacity>
              </View>
...
function DaGui({ navigation }) {
  ...
  async function getSentFriendRequests() {
    try {
      const res = await axiosPrivate.get(`/friendRequest/sent/${currentUser.user.uid}`);
      const sort = res.sort((a, b) => {
        return new Date(b.createdAt) - new Date(a.createdAt);
      });
      setdataDaGui(sort);
        ...
        renderItem={({ item }) => (
          <View style={{backgroundColor:'#fff', flexDirection: "row",padding:7,width:'100%',height:97, borderBottomWidth:0.5}}>
            ...
            <View style={{ marginLeft: 10,gap:4,width:'50%' }}>
              <Text style={{fontSize:17, fontWeight: "bold" }}>{item.name}</Text>
              <Text style={{ fontSize:15,color:'#767A7F' }}>Tìm kiếm số điện thoại</Text>
              <Text style={{ fontSize:15,color:'#767A7F' }}>{(covertTime(item.createdAt))}</Text>
            </View>
            {item.state === "pending" ? ( <TouchableOpacity onPress={()=>{
                  ThuHoi(item._id)
                
            }} style={{marginTop:18,backgroundColor:'#E0E0E0',width:110,height:32,borderRadius:15,justifyContent:'center',alignItems:'center'}}>
                  <Text style={{color:'black',fontSize:16,fontWeight:500}}>Thu hồi</Text>
                </TouchableOpacity>)
                :( <View style={{marginTop:18,width:110,height:32,borderRadius:15,justifyContent:'center',alignItems:'center'}}>
